"use client";

import { ReportItem } from "@/types/report";
import { getReportStatusInfo, formatDateTime } from "@/utils/format";
import { ChevronRight } from "lucide-react";

interface Props {
  report: ReportItem;
  onClick?: (id: number) => void;
}

export default function ReportItemCard({ report, onClick }: Props) {
  const { label, style } = getReportStatusInfo(report.reportStatus);

  return (
    <div 
      onClick={() => onClick?.(report.reportId)}
      className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm flex items-center justify-between group active:scale-[0.98] transition-all cursor-pointer"
    >
      <div className="flex-1">
        <div className="flex items-center gap-2 mb-1">
          <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${style}`}>
            {label}
          </span>
          <span className="text-[11px] text-gray-400 font-medium">
            {formatDateTime(report.reportedAt)}
          </span>
        </div>
        <p className="font-bold text-gray-800">킥보드 ID: {report.scooterId}</p>
        <p className="text-sm text-gray-400 mt-0.5">신고 번호: {report.reportId}</p>
      </div>
      <ChevronRight className="text-gray-200 group-hover:text-blue-500 transition-colors" size={20} />
    </div>
  );
}